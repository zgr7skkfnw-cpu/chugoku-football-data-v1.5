...
export function renderScheduledMatchPage({ match, home, away, matches, teamDirectory, playerDirectory, playerStatistics, competition, selectedMatchTab = "preview" }) {
  ...
  tabs.append(...definitions.map(([key, label]) => {
    ...
    const button = element("button", { className: `prematch-tab${selected ? " is-active" : ""}`, text: label, attributes: { id: `prematch-tab-${key}`, type: "button", role: "tab", "aria-selected": String(selected), "aria-controls": `prematch-panel-${key}`, tabindex: selected ? "0" : "-1", "data-prematch-tab": key } });
  ...
  const content = element("div", { className: "prematch-tab-content", attributes: { id: `prematch-panel-${activeTab}`, role: "tabpanel", "aria-labelledby": `prematch-tab-${activeTab}`, "data-prematch-content": activeTab } });
...
function suspensionsTab({ match, home, away, matches }) {
  const suspensions = match.suspensions;
  if (!suspensions?.home?.length && !suspensions?.away?.length) {
    const automatic = calculateAutomaticSuspensions(match, matches, home, away);
    if (automatic.supported) return element("div", { className: "suspension-grid", attributes: { "data-suspensions": "automatic" } }, automatic.sides.map(({ team, items }) => createPanel(team?.name ?? "チーム", items.length ? element("div", { className: "detail-list" }, items.map((item) => element("div", { className: "suspension-row" }, [element("strong", { text: item.playerName }), element("span", { text: item.reason }), element("small", { text: item.source })]))) : createNotice("公式記録に基づく自動停止対象は確認されませんでした。大会規律委員会による追加処分は含みません。"), "大会要項による自動判定")));
    return element("div", { className: "section-stack", attributes: { "data-suspensions": "unpublished" } }, [createNotice("出場停止情報は公式記録未掲載です。この大会は自動判定規定を確認できていないため、自動判定対象外です。")]);
  }
...
export function renderMatchInsights(forms, home, away, matches, match) {
  ...
  for (const { team, items } of forms) {
    ...
    if (!insights.some((item) => item.teamId === team?.id || item.text.startsWith(team?.shortName ?? team?.name ?? ""))) insights.push({ teamId: team?.id, text: `${team?.shortName ?? team?.name}は直近${items.length}試合で${wins}勝${draws}分${losses}敗`, basis: `直近${items.length}試合`, priority: 5 });
  }
...
function topScorerPanel({ match, home, away, matches, playerDirectory, playerStatistics, teamDirectory }) {
  ...
  const leaders = [home?.id, away?.id].map((teamId) => [...totals.values()].filter((item) => item.teamId === teamId).sort((a, b) => b.goals - a.goals || b.assists - a.assists || a.name.localeCompare(b.name, "ja"))[0]).filter((item) => item?.goals > 0);
  if (!leaders.length) return createNotice("得点ランキングは公式記録公開後に表示します。");
  return createPanel("両チームの今大会最多得点者", element("div", { className: "prematch-scorer-list" }, leaders.map((leader) => { const player = getPlayer(playerDirectory, leader.name, leader.teamId); const stats = player ? playerStatistics?.get(player.id) : null; const appearances = stats?.matches?.filter((item) => matchIds.has(item.matchId) && item.minutes > 0) ?? []; const minutes = appearances.length ? appearances.reduce((sum, item) => sum + item.minutes, 0) : null; return element(player ? "a" : "article", { className: "prematch-scorer", attributes: { ...(player ? { href: routeHref("player", { playerId: player.id }), "data-route": "player" } : {}), "data-player-id": player?.id ?? "", "data-team-id": leader.teamId } }, [element("strong", { text: leader.name }), element("span", { text: getTeam(teamDirectory, leader.teamId)?.name ?? leader.teamId }), metric("背番号", player?.number ?? "－"), metric("ゴール", leader.goals), metric("アシスト", leader.assists), metric("シュート", "－"), metric("出場", appearances.length || "－"), metric("出場時間", minutes == null ? "－" : `${minutes}分`), metric("90分当たり", minutes > 0 ? (leader.goals * 90 / minutes).toFixed(2) : "－")]); })), "当該大会・試合日まで");
}
...
function calculateAutomaticSuspensions(target, matches, home, away) {
  if (!/^jufa-chugoku-2026-division-[12]$/.test(target.competitionId ?? "")) return { supported: false, sides: [] };
  const prior = previousMatches(matches, target).filter((item) => item.competitionId === target.competitionId).sort((a, b) => new Date(a.kickoffAt) - new Date(b.kickoffAt));
  const sides = [home, away].map((team) => {
    const teamMatches = prior.filter((item) => [item.homeTeam.teamId, item.awayTeam.teamId].includes(team?.id));
    const latest = teamMatches.at(-1); const yellowCounts = new Map(); const items = [];
    for (const item of teamMatches) { const side = item.homeTeam.teamId === team?.id ? "home" : "away"; for (const raw of item.disciplinary?.[side] ?? []) { const parsed = disciplinaryRecord(raw); if (parsed.type === "yellow") yellowCounts.set(parsed.playerName, (yellowCounts.get(parsed.playerName) ?? 0) + 1); if (item === latest && parsed.type === "red") items.push({ playerName: parsed.playerName, reason: "直前の試合で退場（次戦1試合停止）", source: "2026年度中国大学サッカーリーグ大会要項 11(8)" }); } }
    if (latest) { const side = latest.homeTeam.teamId === team?.id ? "home" : "away"; for (const raw of latest.disciplinary?.[side] ?? []) { const parsed = disciplinaryRecord(raw); if (parsed.type === "yellow" && yellowCounts.get(parsed.playerName) % 3 === 0) items.push({ playerName: parsed.playerName, reason: `警告累積${yellowCounts.get(parsed.playerName)}回（次戦停止）`, source: "2026年度中国大学サッカーリーグ大会要項 11(9)" }); } }
    return { team, items: [...new Map(items.map((item) => [`${item.playerName}:${item.reason}`, item])).values()] };
  });
  return { supported: true, sides };
}

function disciplinaryRecord(raw = "") { const text = String(raw); const code = text.match(/\s(CS|C\d+|S\d+)\s/)?.[1] ?? ""; const prefix = text.match(/^(?:\d+(?:\s*[＋+]\s*\d+)?\s*分|HT)\s*(.*)$/)?.[1] ?? text; const playerName = (code ? prefix.slice(0, prefix.indexOf(code)) : prefix).trim(); return { playerName, type: code === "CS" || code.startsWith("S") ? "red" : "yellow" }; }
