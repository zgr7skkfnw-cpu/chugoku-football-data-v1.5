...
export function renderFollowingPage({ matches, teamDirectory, playerDirectory, favoriteTeamId, favoritePlayerIds = [] }) {
  let activeTab = "teams";
  ...
  const createTeamCards = () => {
    ...
    if (!team) return createNotice("フォロー中のチームはありません。チームページまたは試合タブから登録してください。");
    const next = matches.filter((match) => includesTeam(match, team.id) && match.status !== "finished" && new Date(match.kickoffAt) >= new Date()).sort((a, b) => new Date(a.kickoffAt) - new Date(b.kickoffAt))[0];
    return element("div", { className: "following-card-grid" }, [createTeamCard(team, next, teamDirectory)]);
  };
...
function createTeamCard(team, next, teamDirectory) {
  ...
  const card = element("a", { className: `following-team-card${dark ? " is-dark" : ""}`, attributes: { href: routeHref("team", { teamId: team.id }), "data-route": "team", "data-team-id": team.id } }, [
